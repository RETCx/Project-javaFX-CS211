...
public class UserList {
    ...
    public void addUser(String username,String password ,String name) {
        ...
        name = name.trim();
        // จำเป็นต้อง check ไหม เพราะก่อนใส่ก็เช็คอยู่แล้วใน controller
        if (!username.equals("") && !password.equals("") && !name.equals("")) { //เป็นจริงเมื่อ string ไม่เป็นค่าว่าง
            User exist = findUserByUsername(username);
            if (exist == null) {
                users.add(new User(username.trim(), password.trim(),name.trim()));
            }
        }
    }
